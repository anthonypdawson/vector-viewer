...
def test_pinecone_delete_where_and_ids_precedence(mock_pinecone_client):
    """Ensure Pinecone delete uses filter when where provided and ids when given."""
    mock_index = Mock()
    mock_pinecone_client.Index.return_value = mock_index
    ...
    # Delete by where -> should call delete(filter=...)
    res = conn.delete_items("test-index", where={"type": "remove"})
    assert res is True
    assert mock_index.delete.called
    # Inspect call to ensure 'filter' kwarg was used
    called_kwargs = mock_index.delete.call_args[1]
    assert "filter" in called_kwargs

    mock_index.reset_mock()

    # Delete by ids (with where present) -> ids should take precedence
    res2 = conn.delete_items("test-index", ids=["idA"], where={"type": "keep"})
    assert res2 is True
    assert mock_index.delete.called
    called_kwargs2 = mock_index.delete.call_args[1]
    assert "ids" in called_kwargs2
