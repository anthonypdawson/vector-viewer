...
def test_qdrant_delete_where_and_ids_precedence(tmp_path):
    collection_name = f"test_collection_{uuid.uuid4().hex[:8]}"
    test_ids = ["id1", "id2"]
    test_vectors = [[0.1, 0.2], [0.3, 0.4]]
    test_docs = ["hello", "world"]
    test_metadata = [{"type": "keep"}, {"type": "remove"}]
    ...
    if not deleted:
        pytest.skip("Qdrant delete-by-where not supported in this environment")

    res = conn.get_all_items(collection_name, limit=10)
    if not res or not res.get("documents"):
        pytest.skip("Qdrant get_all_items not supported in this environment")
